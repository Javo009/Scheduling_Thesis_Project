...
double schedule_messages(
    int currentTaskId,
    int candidateProcId,
    const vector<int> &sortedPredecessors,
    unordered_map<int, LinkSchedule> &dummyLinkSchedules
) {
    double commFinish = 0.0;
    ...
    for (int predId : sortedPredecessors) {
        ...
        const auto &pathNodes = shortest_path[predProcId][candidateProcId];
        if (pathNodes.empty() && (predProcId!=candidateProcId)) {
            return -1.0;
        }

        if(predProcId==candidateProcId){
            commFinish = max(commFinish, msgStart);
            continue;
        }
        ...
        for (size_t i = 0; i + 1 < pathNodes.size(); i++) {
            ...
            long long key = makeKey(fromNode, toNode);
            if (!linkMap.count(key)) {
                return -1.0;
            }
            int linkId = linkMap.at(key);

            double bandwidth = linkById.at(linkId).bandwidth;
            
            double duration;

            duration = msgSize / bandwidth + nodeStartupDelay[fromNode];
            
            double earliestSlot = ephemeral_reserveLink(
                dummyLinkSchedules[linkId], 
                msgStart, 
                duration
            );
            if (earliestSlot < 0) {
            ...
            msgStart = earliestSlot + duration;
        }
        commFinish = max(commFinish, msgStart);
    }
    return commFinish;
...
//-============= Using Flloyd Marshall to get all pairs shortest paths==================

    nextHop.assign(nodeCount + 1, vector<int>(nodeCount + 1, -1));
    dist.assign(nodeCount + 1, vector<Cost>(nodeCount + 1, Cost{0.0, INF}));
...
//-================ Taking care of the preloading==========================
    ...
    while(!taskList.empty()){
        ...
        for(auto p: processors){
            ...
            if(current_task.predecessors.size()==0){
                ...
                max_comm_time[p.id] = schedule_messages(current_task.id, p.id, message_priority_list, dummyLinkSchedule[p.id]);
    ...
    // for(auto it: finishTimeOfTask)
    // cout << it.second << " ";

    cout << "Makespan: " << finishTimeOfTask[exit_task_id] << "\n";
